...
const navigation = [
  ...
  { label: "Bookings", icon: "file", href: "/bookings", permission: "bookings.view" },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="app-layout">
      <button className={`scrim ${menuOpen ? "visible" : ""}`} onClick={closeMenu} aria-label="Close navigation" />
      <aside className={`sidebar ${menuOpen ? "open" : ""}`}>
        <div className="sidebar-head">
          <div className="brand"><span className="brand-mark" aria-hidden="true">E</span><span>EstateOps</span></div>
          <button className="icon-button sidebar-close" onClick={closeMenu} aria-label="Close navigation">×</button>
        </div>
        ...
        <nav className="main-nav" aria-label="Primary navigation">
          ...
          {navigation.filter((item) => !item.permission || hasPermission(item.permission)).map((item) => {
            const enabled = Boolean(item.href && (!item.permission || hasPermission(item.permission)));
            if (enabled && item.href) {
              return (
                <Link key={item.label} href={item.href} onClick={closeMenu} className={pathname === item.href || (item.href === "/site-visits" && pathname === "/calendar") || (["/leads", "/customers", "/projects", "/site-visits", "/quotations", "/documents", "/bookings", "/collections", "/partners", "/post-sales", "/property-lifecycle"].includes(item.href) && pathname.startsWith(`${item.href}/`)) ? "active" : ""}>
                  <Icon name={item.icon} /><span>{item.label}</span>
                </Link>
              );
            }
            return (
              <button key={item.label} disabled title="Available in a later delivery phase">
                <Icon name={item.icon} /><span>{item.label}</span><small>Soon</small>
              </button>
            );
          })}
        </nav>
        <div className="sidebar-bottom">
          {hasPermission("organization.view") && (
            <Link href="/settings/organization" onClick={closeMenu} className={pathname.startsWith("/settings/") && pathname !== "/settings/roles" ? "active" : ""}>
              <Icon name="building" /><span>Organization settings</span>
            </Link>
          )}
          {hasPermission("roles.view") && (
            <Link href="/settings/roles" onClick={closeMenu} className={pathname === "/settings/roles" ? "active" : ""}>
              <Icon name="shield" /><span>Roles & permissions</span>
            </Link>
          )}
          <Link href="/account/security" onClick={closeMenu} className={pathname === "/account/security" ? "active" : ""}>
            <Icon name="settings" /><span>Account security</span>
          </Link>
